...
class TestZoneTrackingCollisions:
    ...
    @staticmethod
    def _make_clusters_with_maps(zone_maps: dict[str, str]) -> ClusterPool:
        pool = ClusterPool()
        pool.zone_maps = zone_maps
        return pool

    def test_shared_exit_gate_same_entrance_map_rejected(self):
        """Two edges sharing exit fog_id with same entrance map = error."""
        dag = Dag(seed=42)
        dag.add_node(
            DagNode(
                id="start",
        ...
        dag.add_node(
            DagNode(
                ...
                cluster=make_cluster(
                    "cave",
                    zones=["cave_main", "cave_boss"],
                    cluster_type="mini_dungeon",
                ),
                layer=1,
                tier=1,
                entry_fogs=[FogRef("entry_fog", "cave_main")],
            )
        )
        dag.add_node(
            DagNode(
                ...
                tier=1,
                entry_fogs=[
                    FogRef("castle_entry_a", "castle_front"),
                    FogRef("castle_entry_b", "castle_front"),
                ],
            )
        )
        # Two edges from different zones of the cave, same exit fog_id,
        # both targeting castle_front (same map)
        dag.add_edge(
            "start",
            "mid",
            FogRef("start_exit", "start_c_zone"),
            FogRef("entry_fog", "cave_main"),
        )
        dag.add_edge(
            "mid",
            "end_a",
            FogRef("AEG099_003_9000", "cave_main"),
            FogRef("castle_entry_a", "castle_front"),
        )
        dag.add_edge(
            "mid",
            "end_a",
            FogRef("AEG099_003_9000", "cave_boss"),
            FogRef("castle_entry_b", "castle_front"),
        )
        dag.start_id = "start"
        dag.end_id = "end_a"
        ...
        clusters = self._make_clusters_with_maps(
            {
                "cave_main": "m31_15_00_00",
                "cave_boss": "m31_15_00_00",
                "castle_front": "m10_00_00_00",
        ...
        assert "AEG099_003_9000" in collision_errors[0]

    def test_shared_exit_gate_different_entrance_maps_accepted(self):
        """Two edges sharing exit fog_id with different entrance maps = ok."""
        dag = Dag(seed=42)
        dag.add_node(
            DagNode(
                id="start",
        ...
        dag.add_node(
            DagNode(
                ...
                layer=2,
                tier=1,
                entry_fogs=[FogRef("castle_entry", "castle_zone")],
            )
        )
        dag.add_node(
            DagNode(
                ...
                layer=2,
                tier=1,
                entry_fogs=[FogRef("ruins_entry", "ruins_zone")],
            )
        )
        dag.add_edge(
            "start",
        ...
        # Same exit fog_id but different entrance maps
        dag.add_edge(
            "mid",
            "end_a",
            FogRef("AEG099_003_9000", "cave_main"),
            FogRef("castle_entry", "castle_zone"),
        )
        dag.add_edge(
            "mid",
            "end_b",
            FogRef("AEG099_003_9000", "cave_boss"),
            FogRef("ruins_entry", "ruins_zone"),
        )
        dag.start_id = "start"
        dag.end_id = "end_a"
        ...
        clusters = self._make_clusters_with_maps(
            {
                "cave_main": "m31_15_00_00",
                "cave_boss": "m31_15_00_00",
                "castle_zone": "m10_00_00_00",
                "ruins_zone": "m12_05_00_00",
                "start_c_zone": "m10_01_00_00",
            }
        )
        config = make_config(legacy_dungeons=0, bosses=0, mini_dungeons=0)
    ...
    def test_compound_collision_same_source_same_entrance_map(self):
        """Two edges from same node to same entrance map = compound error."""
        dag = Dag(seed=42)
        dag.add_node(
            DagNode(
                id="start",
        ...
        dag.add_node(
            DagNode(
                ...
                layer=1,
                tier=1,
                entry_fogs=[FogRef("entry_fog", "boss_zone")],
            )
        )
        dag.add_node(
            DagNode(
                ...
                layer=2,
                tier=1,
                entry_fogs=[FogRef("castle_a_entry", "castle_a_zone")],
            )
        )
        dag.add_node(
            DagNode(
                ...
                layer=2,
                tier=1,
                entry_fogs=[FogRef("castle_b_entry", "castle_b_zone")],
            )
        )
        dag.add_edge(
            "start",
            "mid",
            FogRef("start_exit", "start_c_zone"),
            FogRef("entry_fog", "boss_zone"),
        )
        # Two edges from mid, using different exit gates,
        # but both targeting zones on the SAME map (m15_00)
        dag.add_edge(
            "mid",
            "end_a",
            FogRef("AEG099_001_9000", "boss_zone"),
            FogRef("castle_a_entry", "castle_a_zone"),
        )
        dag.add_edge(
            "mid",
            "end_b",
            FogRef("AEG099_001_9001", "boss_zone"),
            FogRef("castle_b_entry", "castle_b_zone"),
        )
        dag.start_id = "start"
        dag.end_id = "end_a"

        # castle_a_zone and castle_b_zone map to the SAME map
        clusters = self._make_clusters_with_maps(
            {
                "boss_zone": "m12_05_00_00",
                "castle_a_zone": "m15_00_00_00",
                "castle_b_zone": "m15_00_00_00",
                "start_c_zone": "m10_01_00_00",
            }
        )
        config = make_config(legacy_dungeons=0, bosses=0, mini_dungeons=0)
        ...
        compound_errors = [e for e in result.errors if "Compound collision" in e]
        assert len(compound_errors) == 1
        assert "mid" in compound_errors[0]
        assert "m15_00_00_00" in compound_errors[0]

    def test_compound_no_collision_different_entrance_maps(self):
        """Two edges from same node to different entrance maps = ok."""
        dag = Dag(seed=42)
        dag.add_node(
            DagNode(
                id="start",

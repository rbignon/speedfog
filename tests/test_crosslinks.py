...
class TestCollisionAwareCrosslinks:
    """Tests for collision-aware cross-link filtering (fog_id-based)."""

    def _make_pool(self, zone_maps: dict[str, str]) -> ClusterPool:
        pool = ClusterPool()
        pool.zone_maps = zone_maps
        return pool

    def test_build_collision_index(self):
        """Index maps exit fog_id → set of entrance maps."""
        dag = Dag(seed=42)
        c_a = make_cluster(
            "a",
            entry_fogs=[{"fog_id": "a_e", "zone": "a"}],
            exit_fogs=[{"fog_id": "shared_gate", "zone": "a"}],
        )
        c_b = make_cluster(
            "b",
            entry_fogs=[{"fog_id": "b_e", "zone": "b"}],
            exit_fogs=[{"fog_id": "b_x", "zone": "b"}],
        )
        dag.nodes["a"] = DagNode("a", c_a, layer=0, tier=1)
        dag.nodes["b"] = DagNode("b", c_b, layer=1, tier=1)
        dag.add_edge("a", "b", FogRef("shared_gate", "a"), FogRef("b_e", "b"))

        pool = self._make_pool({"a": "m10", "b": "m20"})
        index = _build_collision_index(dag, pool)
        assert index == {"shared_gate": {"m20"}}

    def test_would_collide_same_fog_id_same_dest_map(self):
        """Collision when fog_id already targets the same entrance map."""
        index = {"shared_gate": {"m20"}}
        pool = self._make_pool({"c": "m20"})
        assert _would_collide(
            FogRef("shared_gate", "x"), FogRef("c_e", "c"), index, pool
        )

    def test_would_collide_different_fog_id(self):
        """No collision when fog_id differs."""
        index = {"shared_gate": {"m20"}}
        pool = self._make_pool({"c": "m20"})
        assert not _would_collide(
            FogRef("other_gate", "x"), FogRef("c_e", "c"), index, pool
        )

    def test_would_collide_different_dest_map(self):
        """No collision when dest map differs."""
        index = {"shared_gate": {"m20"}}
        pool = self._make_pool({"c": "m30"})
        assert not _would_collide(
            FogRef("shared_gate", "x"), FogRef("c_e", "c"), index, pool
        )

    def test_crosslink_skips_colliding_fog_id(self):
        """Cross-link picks non-colliding exit fog when one would collide.

        Setup: s(0) → a(1) → end(2), s(0) → b(1) → c(2) → end(3)
        Cross-link candidate: a(1) → c(2).
        A has two surplus exits: shared_gate (collides) and unique_gate (safe).
        Edge d→c already uses shared_gate to reach c's map (m30).
        """
        dag = Dag(seed=42)

        c_start = make_cluster(
            "s",
            "start",
            weight=0,
            entry_fogs=[],
        ...
        c_a = make_cluster(
            ...
            entry_fogs=[{"fog_id": "a_e", "zone": "a"}],
            exit_fogs=[
                {"fog_id": "a_x_main", "zone": "a"},
                {"fog_id": "shared_gate", "zone": "a"},
                {"fog_id": "unique_gate", "zone": "a"},
            ],
        )
        c_b = make_cluster(
            "b",
        ...
        c_c = make_cluster(
            "c",
            entry_fogs=[{"fog_id": "c_e", "zone": "c"}],
            exit_fogs=[{"fog_id": "c_x", "zone": "c"}],
        )
        c_end = make_cluster(
            "end",
            "boss_arena",
            weight=3,
            entry_fogs=[
                {"fog_id": "e_e1", "zone": "end"},
                {"fog_id": "e_e2", "zone": "end"},
                {"fog_id": "e_e3", "zone": "end"},
            ],
            exit_fogs=[],
        )

        dag.nodes["s"] = DagNode("s", c_start, layer=0, tier=1)
        dag.nodes["a"] = DagNode("a", c_a, layer=1, tier=1)
        dag.nodes["b"] = DagNode("b", c_b, layer=1, tier=1)
        dag.nodes["c"] = DagNode("c", c_c, layer=2, tier=1)
        dag.nodes["end"] = DagNode("end", c_end, layer=3, tier=1)
        dag.start_id = "s"
        dag.end_id = "end"

        dag.add_edge("s", "a", FogRef("s_x1", "s"), FogRef("a_e", "a"))
        dag.add_edge("s", "b", FogRef("s_x2", "s"), FogRef("b_e", "b"))
        dag.add_edge("a", "end", FogRef("a_x_main", "a"), FogRef("e_e1", "end"))
        dag.add_edge("b", "c", FogRef("b_x", "b"), FogRef("c_e", "c"))
        dag.add_edge("c", "end", FogRef("c_x", "c"), FogRef("e_e2", "end"))
        ...
        # Edge d→c uses shared_gate → m30 (c's map)
        c_dummy = make_cluster(
            "d",
            entry_fogs=[{"fog_id": "d_e", "zone": "d"}],
            exit_fogs=[{"fog_id": "shared_gate", "zone": "d"}],
        )
        dag.nodes["d"] = DagNode("d", c_dummy, layer=0, tier=1)
        dag.add_edge("d", "c", FogRef("shared_gate", "d"), FogRef("c_e", "c"))

        # shared_gate already → m30. Cross-link a→c via shared_gate collides.
        # unique_gate → m30 is fine (new fog_id, no prior entry for it).
        added = add_crosslinks(dag, rng=random.Random(42), clusters=pool)

        assert added >= 1
        crosslink_edges = [
            e for e in dag.edges if e.source_id == "a" and e.target_id == "c"
        ]
        for e in crosslink_edges:
            assert (
                e.exit_fog.fog_id != "shared_gate"
            ), f"Cross-link used colliding fog_id: {e.exit_fog.fog_id}"

    def test_crosslink_skipped_when_all_combos_collide(self):
        """Cross-link skipped when every exit fog_id would collide.

        Same structure, but A only has shared_gate as surplus — no alternative.
        """
        dag = Dag(seed=42)
        ...
        # A's only surplus (shared_gate) collides → no cross-link possible
        added = add_crosslinks(dag, rng=random.Random(42), clusters=pool)
        assert added == 0

#!/bin/bash
# SpeedFog Launcher for Elden Ring (Linux/Proton via ME3)
...
if [ "$enabled" != "false" ]; then
    ...
    if [ -n "$SAVE_PATH" ]; then
        echo "Starting backup daemon for: $SAVE_PATH"
        nohup "$SCRIPT_DIR/backup_daemon.sh" "$SAVE_PATH" > /dev/null 2>&1 &
    fi
fi

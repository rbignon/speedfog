...
| `item_preset_path` | `item_randomizer.item_preset` | Optional YAML controlling item placement. The preset (`item_randomizer.item_preset_path`, relative to the working directory; empty = `data/item_preset.yaml`) must exist: a missing one fails the generation (`speedfog/main.py` before any work, `ItemRandomizerWrapper` when handed a missing file) instead of silently falling back to the randomizer's built-in default preset |
...
| `dlcblessing` | `true` | GUI default ("Place Shadow Realm Blessings in DLC only"), pinned because v0.12 keys it on a `Switch:` in `annotations.txt` and unset booleans are false headless. Inert with the shipped presets: the switch only filters the randomizer's default preset (`ProcessBuiltins`), and each shipped preset defines its own `DlcOnlyItems` section, whose item list replaces the default one in `Resolve()` (`EditItems=true`). `item_preset.yaml` and `item_preset_without_weapons.yaml` leave it empty; `uwyg_item_preset.yaml` lists both blessings, so they are always DLC-only there. Blessing placement is otherwise governed by the presets' `ShadowRealmBlessings` section, `Mode: Anywhere` since 2026-08-31 so the (SpEffect-neutralized, see `ScaduBlessingNeutralizer`) blessings scatter into generic slots instead of boss drops |
...
## Item Preset Placement

The item presets (`data/item_preset.yaml`, `data/item_preset_without_weapons.yaml`, `data/uwyg_item_preset.yaml`) are parsed by `ItemPreset.ParsePreset()` and turned into per-item `RequireTags`/`ExcludeTags` in `AnnotationData`, applied by `Permutation.AssignItemsToLocations` (`../SoulsRandomizers/RandomizerCommon/Item/`). Rules that matter when listing items, verified against real wrapper runs (2026-09-18):

- **Items and locations are both editable only in `CoreMechanics` and user-defined `Type: Custom` placements.** Items under types with `EditItems=false` (KeyItems, QuestItems, SacredFlaskUpgrades, UpgradeBellBearings, ShadowRealmBlessings) are replaced by the defaults in `Resolve()`; SecondaryMechanics, BossTools, BaseOnlyItems and DlcOnlyItems keep their items but get the default locations. A `Custom` placement has `AllowMissable`, so `missable` must be written in its `Excludes` (it is not auto-added).
- **An item listed in several placements gets the union of their Requires and the union of their Excludes** (`AnnotationData`, `AddMulti` into `RequireTags`/`ExcludeTags`). A second placement can therefore narrow one item of a type whose items are fixed, but only through its Excludes: `item_preset_without_weapons.yaml` lists Sacred Tear again in a `Type: Custom` placement that excludes the other flask-upgrade tags, which leaves Sacred Tears only the `boss` slots (over 4 seeds, all 48 (12 per seed) on major bosses without the Rune Arcs, 47 of 48 with them; Golden Seeds unchanged).
- **Dropping an item from a placement does not keep it away.** It becomes unconstrained, and unconstrained items fill the key slots the constrained ones leave free, at random (the `copydrops` option also adds a finite copy of every enemy-only weapon and armor piece to that pool). Removing the curated weapons from CoreMechanics still left ~5 weapons per seed on key locations (~2 once the `Boss consumables` filled most free key slots). Keeping a group away takes an exclusion (strong in practice, though items still queued after the 5 overflow passes below fall back to random slots): `item_preset_without_weapons.yaml` lists `Group: weapons` in a `Type: Custom` placement whose Excludes are the key and boss tags (safe because no key or quest item is a weapon; an Important item that the Excludes rule out would fail the whole placement). Over 4 seeds this left no weapon on key locations or boss drops; arrows and bolts, a separate `arrows` group, still appear.
- **Restrictions are per item ID, so non-unique items are fine but all-or-nothing.** Listing `goods:1420` constrains every Magic Grease source in the world. The preset cannot limit the copy count or set a quantity: each copy keeps the quantity of its vanilla pickup.
- **Overflow mostly stacks rather than evicts.** Once the eligible slots are taken, restricted items are added to them, up to the lot's `MaxSlots` (at most 6, `IsAvailableSlot`). Past that, the slot's first item is evicted (never a key item; a restricted victim is re-queued), and whatever is still queued after 5 passes falls back to random slots. Adding ~250 consumable copies to the CoreMechanics tags (an early, larger version of `Boss consumables` in `item_preset_without_weapons.yaml`) left talismans, crystal tears and Golden Seeds/Sacred Tears on eligible slots at the baseline rate over 4 seeds, and every consumable copy landed on an eligible slot (the few left elsewhere are shop rows, which never move); boss drops went from 2 to 3 items typically. At ~300 copies (the 62 Rune Arcs added), the consumables still all land on eligible slots, but ~2 of the 55 Golden Seeds/Sacred Tears per seed end up evicted to random slots (1 to 4 over 3 seeds, vs 0 to 1 without the Rune Arcs). Restricting the Rune Arcs, or all added items, to boss-only tags did not reduce that. Giving them an extra non-boss tag made them leave the boss drops almost entirely (~1-3% of bosses instead of ~30%) with each tag set tried (`chest`, `chest`+`scarab`+`legendary`, `legendary`, `talisman`), so none of them gives an in-between rate.
- **Infinite shops are a separate silo.** Infinite-shop copies stay in shops, so an item sold only in infinite shops (Boiled Crab, Boiled Prawn) can never reach a boss slot through the preset. Limited-stock shop rows (`Nx` with a stock flag, such as Moore's pickled livers) are finite and do move, except for items in the randomizer's `norandomshop` group (`diste/Base/annotations.txt`: Rune Arc, Stonesword Key, Fan Daggers), whose shop rows stay put. Craft-only items (crafting "shops" are `norandom`) have no source to move.
- **Ambiguous names need `ID:`.** `Name:` is only a label when `ID:` is set; a name-only entry fails `Resolve()` when the name maps to several items (Golden Vow item `goods:2003170` vs incantation `goods:6600`, Gourmet Scorpion Stew `goods:2001201`/`goods:2001203`).

...
Overlays the SpeedFog Racing badge (the charter wordmark + brass route underline, centered above the logo) onto the vanilla ELDEN RING title screen artwork. Applied at setup by GamePatcher (`TitleScreenPatcher`), not per-seed, and shipped as ~1.5 MB of patched files instead of the 112 MB a naive atlas replacement would cost.

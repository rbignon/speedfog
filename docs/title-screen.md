...
Overlays the SpeedFog Racing badge (the charter wordmark + brass route underline, centered above the logo) onto the vanilla ELDEN RING title screen artwork. Applied at setup by StaticModBuilder (`TitleScreenPatcher`), not per-seed, and shipped as ~1.5 MB of patched files instead of the 112 MB a naive atlas replacement would cost.
...
`TitleScreenPatcher` does this per variant (hi and low) at setup, reading only vanilla files from the game dir: parse the sprite rect from the layout (this also absorbs the hi/low packing difference and future repacks), extract the sprite's BC7 blocks from the atlas (`DdsAtlas.ExtractBlocks`, read-only), decode, composite `data/title_screen_overlay.png`, re-encode, wrap as standalone DDS (`DdsAtlas.BuildStandaloneDds`), rewrite the sblytbnd and 02_title into `data/mods/speedfog/menu/{hi,low}/`. hi and low sprite pixels are identical, so the encode runs once (cached on pixel content).
...
The committed artwork is `data/title_screen_overlay.png`, a **2532x1532 RGBA overlay, transparent except the badge** (the vanilla TM glyph is left untouched now that the badge no longer takes its spot): the speedfog-racing charter wordmark, `SPEEDFOG` in Barlow Condensed 700 off-white `#e8e6e1` and `RACING` in 600 brass `#c8a44e`, centered above the ELDEN RING lettering with the brass hero route underline (triangle, gap, line, station ring, line, gap, terminal square) beneath. It is generated by `tools/generate_title_screen.py` (deterministic on a fixed Pillow environment; standalone from the project venv; the Barlow Condensed TTFs come from the sibling speedfog-racing checkout, `server/speedfog_racing/static/fonts/`): edit that script and re-run it to change the design.
...
2. Re-run the static mod generation:

```bash
cd writer/StaticModBuilder
wine publish/win-x64/StaticModBuilder.exe <game-dir> ../../data/mods/speedfog --data-dir ../../data
```
...
3. Rebuild or re-copy a seed output to get the new static mod files.

## Verification

```bash
# layout: the sprite entry must be gone from SB_Title_01.layout
wine tools/witchybnd/WitchyBND.exe -p data/mods/speedfog/menu/hi/01_common.sblytbnd.dcx
grep -c MENU_Title_EldenRing_01 data/mods/speedfog/menu/hi/01_common-sblytbnd-dcx/SB_Title_01.layout  # 0

# texture: 02_title must contain the composited standalone texture
wine tools/witchybnd/WitchyBND.exe -p data/mods/speedfog/menu/hi/02_title.tpf.dcx
python3 -c "from PIL import Image; Image.open(
    'data/mods/speedfog/menu/hi/02_title-tpf-dcx/MENU_Title_EldenRing_01.dds').convert('RGB').save('/tmp/title.png')"
```

If `data/mods/speedfog/menu/{hi,low}/01_common.tpf.dcx` exists, it is a stale full-atlas override from a pre-redirect SpeedFog version; the patcher deletes it on its next run, and it is safe to delete by hand.

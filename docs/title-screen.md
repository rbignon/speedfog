...
The committed artwork is `data/title_screen_overlay.png`, a **2532x1532 RGBA overlay, transparent except the badge** (the vanilla TM glyph is left untouched now that the badge no longer takes its spot): the speedfog-racing charter wordmark, `SPEEDFOG` in Barlow Condensed 700 off-white `#e8e6e1` and `RACING` in 600 brass `#c8a44e`, centered above the ELDEN RING lettering with the brass hero route underline (triangle, line, station ring, line, terminal square) beneath. It is generated by `tools/generate_title_screen.py` (deterministic on a fixed Pillow environment; standalone from the project venv; the Barlow Condensed TTFs come from the sibling speedfog-racing checkout, `server/speedfog_racing/static/fonts/`): edit that script and re-run it to change the design.

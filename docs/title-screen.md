...
The committed artwork is `data/title_screen_overlay.png`, a **2532x1532 RGBA overlay, transparent except the badge** (plus an opaque black patch erasing the vanilla TM glyph, whose spot the badge takes): the wordmark in the site's gold #c8a44e and a checkered flag dissolving into fog toward the right. It is generated by `tools/generate_title_screen.py` (deterministic on a fixed Pillow + fonts-inter environment; standalone from the project venv): edit that script and re-run it to change the design.

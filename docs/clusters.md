...
- `boss_name`: canonical boss name from ItemRandomizer's `enemy.txt` (via `DefeatFlag -> Important.Names.Key` mapping, preferred over the legacy `ExtraName`). Only present on boss clusters with a matching defeat_flag. Phase suffixes are stripped ("Fire Giant 2" -> "Fire Giant"). Used by the racing server for consistent stats naming across seeds with and without boss randomization. When boss randomization is active, `patch_graph_boss_placements()` in `main.py` overrides `boss_name` on graph nodes with the randomized boss's name, resolved through the same `Names.Key` source (then `ExtraName`, then `boss_arena_tags.json` name as fallback), so naming is consistent across seeds regardless of whether boss randomization is enabled.

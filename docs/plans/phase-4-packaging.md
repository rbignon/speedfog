...
```bash
# 1. Generate the graph (Python)
speedfog config.toml -o graph.json

# 2. Generate the complete mod (C#)
dotnet run --project writer/SpeedFogWriter -- graph.json "/path/to/ELDEN RING/Game" ./output
...
### 4.4.1: Spoiler Generator

Generate a human-readable text file describing the generated path:

```csharp
public class SpoilerWriter
{
    public static void WriteSpoiler(string outputDir, SpeedFogGraph graph)
    {
        ...
        sb.AppendLine($"  Seed: {graph.Metadata.Seed}");
        ...
        var startNode = graph.Nodes.First(n => n.Type == "start");
        ...
        for (int i = 0; i < paths.Count; i++)
        {
            ...
            foreach (var nodeId in paths[i])
            {
                ...
                var scaling = node.ScalingTier > 0 ? $" [Tier {node.ScalingTier}]" : "";
                sb.AppendLine($"  → {node.Name}{scaling}");
            }
        }
        ...
        foreach (var node in graph.Nodes.OrderBy(n => n.ScalingTier))
        {
            sb.AppendLine();
            sb.AppendLine($"[{node.Name}]");
            sb.AppendLine($"  Zones: {string.Join(", ", node.Zones)}");
            sb.AppendLine($"  Scaling Tier: {node.ScalingTier}");
            sb.AppendLine($"  Boss: {(node.HasBoss ? "Yes" : "No")}");

            var outEdges = graph.Edges.Where(e => e.From == node.Id);
            if (outEdges.Any())
            {
                sb.AppendLine($"  Exits:");
                foreach (var edge in outEdges)
                {
                    var target = graph.Nodes.First(n => n.Id == edge.To);
                    sb.AppendLine($"    → {target.Name} via {edge.FogId}");
                }
            }
        }
...
```csharp
// Usage:
// dotnet run -- <graph.json> <game_path> <output_dir> [options]
//
// Options:
//   --no-package       Skip ModEngine packaging (output mod files only)
//   --update-modengine Force re-download of ModEngine 2
//   --spoiler          Generate spoiler.txt (default: enabled)
//   --no-spoiler       Skip spoiler generation
...
│   └── SpoilerWriter.cs         # Spoiler log generation
│
└── Models/

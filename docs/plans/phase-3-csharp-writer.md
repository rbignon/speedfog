...
- **Data**: `clusters.json` with zone→map mapping in `zone_maps` field (generated by `generate_clusters.py`)
...
│   # Note: zone→map mapping is in clusters.json (zone_maps field)
│
├── SpeedFogWriter/
...
│   │   ├── ClusterFile.cs         # Cluster data with zone_maps (clusters.json)
...
public void LoadMsbsForGraph(SpeedFogGraph graph, FogDataFile fogData)
{
    ...
    foreach (var node in graph.AllNodes())
    {
        ...
        {
            // Lookup from clusters.json zone_maps
            var map = _clusterData.GetMap(zone);
            if (map != null)
...
## Task 3.2.2: Zone Data - REMOVED

**Status**: This task has been removed. Zone→map mapping is now integrated directly into `clusters.json`.

### How to Access Zone→Map Mapping

The `clusters.json` file (version 1.1+) includes a `zone_maps` field at the root level:

```json
{
  "version": "1.1",
  "zone_maps": {
    "chapel_start": "m10_01_00_00",
    "stormveil": "m10_00_00_00",
    "academy": "m14_00_00_00"
  },
  "clusters": [...]
}
```

The C# writer should load this from `clusters.json` instead of a separate `zones_data.json` file.

### C# Implementation

```csharp
// In ClusterFile.cs or similar:
...
**Note**: No extraction script needed. Zone→map data is generated by `tools/generate_clusters.py` directly from fog.txt.
...
- Target cluster's zones → `clusters.json` zone_maps → destination map
...
{
    // Use FogRando's entity ID ranges (no conflict since not used together)
    // Reference: GameDataWriterE.cs L124-135
    private const uint FogEntityBase = 755890000;      // num4 in FogRando
    private const uint FlagBase = 1040290000;          // num5 in FogRando
    private const uint RegionBase = 1040290070;        // num6 = num5 + 70

    private readonly FogDataFile _fogData;
    private readonly ClusterFile _clusterData;  // zone_maps included here
    private uint _nextEntityId;
    private uint _nextFlagId;
    private uint _nextRegionId;

    public FogGateWriter(FogDataFile fogData, ClusterFile clusterData)
    {
        _fogData = fogData;
        _clusterData = clusterData;
        _nextEntityId = FogEntityBase;
        _nextFlagId = FlagBase;
        _nextRegionId = RegionBase;
    }
    ...
    public List<FogGateEvent> CreateFogGates(SpeedFogGraph graph)
    {
        ...
        foreach (var edge in graph.Edges)
        {
            ...
            // Determine target map from clusters.json zone_maps
...
**Strategy** (reusing FogRando's ID ranges, L124-135):
...
public class EntityIdAllocator
{
    ...
    // Reference: GameDataWriterE.cs L124-135
    private uint _nextEntityId = 755890000;    // num4 in FogRando
    private uint _nextFlagId = 1040290000;     // num5 in FogRando
    private uint _nextRegionId = 1040290070;   // num6 = num5 + 70
...
public class ModWriter
{
    ...
    private void LoadSpeedFogData()
    {
        ...
        // Load clusters.json (includes zone_maps)
        var clustersPath = Path.Combine(_dataDir, "clusters.json");
        if (!File.Exists(clustersPath))
            throw new FileNotFoundException($"clusters.json not found: {clustersPath}");
    ...
    {
        if (_fogData == null || _clusterData == null)
            throw new InvalidOperationException("SpeedFog data not loaded");

        var fogWriter = new FogGateWriter(_fogData, _clusterData);
...
| `ZoneMap` | Lookup via `clusters.json` zone_maps |
| `entries`/`exits` (node IDs) | `entry_fogs`/`exit_fogs` (fog ID lists) |
| No explicit edges | `edges` array with fog_id |

The C# writer uses edges, fog_data, and clusters.json to determine:
- Where to place fog walls (source cluster exit fog position from fog_data)
- Where to warp (target cluster map from clusters.json zone_maps, position from entry fog)
...
### Task 3.2.2 (zone→map mapping) - COMPLETE
- [x] Zone→map mapping integrated into `clusters.json` (version 1.1)
- [x] `generate_clusters.py` extracts zone maps from fog.txt
- [x] All zones in clusters have map mappings in `zone_maps` field
...
- [ ] `ClusterFile.Load()` parses clusters.json with zone_maps correctly
...
### Task 3.6 (Warps)
- [ ] Target map determined via clusters.json zone_maps
...
- [ ] Full pipeline works: graph.json + fog_data.json + clusters.json → mod files

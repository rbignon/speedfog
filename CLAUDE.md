...
| `UntouchableBossInjector` | Aging Untouchable minor boss: NpcParam clone + partial damage-cut SpEffect, parry break (a common.emevd event per placed boss cancels the cut after the first parry), boss think row + behavior variation + beam bullet + madness-free lantern pulses when the static assets exist, repoints enemy-randomizer placements (NPCParamID, ThinkParamID), see `docs/untouchable-boss.md` |
